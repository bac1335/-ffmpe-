...
; 安装程序初始定义常量
!define PRODUCT_NAME "cameratest"
!define PRODUCT_VERSION "1.0.0"
!define PRODUCT_PUBLISHER "cameratest"
;!define PRODUCT_WEB_SITE "airlink.timelink.cn"
!define PRODUCT_DIR_REGKEY "Software\Microsoft\Windows\CurrentVersion\App Paths\cameratest.exe"
...
!define CHINESE_NAME "cameratest"
;设置窗口置顶
...
; 安装完成页面
;!define MUI_FINISHPAGE_RUN "$INSTDIR\cameratest.exe"
...
LangString ProgLinkName ${LANG_SIMPCHINESE} "摄像头"
LangString ProgLinkName ${LANG_ENGLISH} "cameratest"
...
InstallDir "$PROGRAMFILES\cameratest"
...
Section "MainSection" SEC01
  ...
  CreateDirectory "$SMPROGRAMS\cameratest"
  CreateShortCut "$SMPROGRAMS\cameratest\$(ProgLinkName).lnk" "$INSTDIR\cameratest.exe"
  CreateShortCut "$DESKTOP\$(ProgLinkName).lnk" "$INSTDIR\cameratest.exe"
  File /r ".\cameratest\*"
...
Section -AdditionalIcons
  ...
  CreateShortCut "$SMPROGRAMS\cameratest\Uninstall.lnk" "$INSTDIR\uninst.exe"
  ; 防火墙规则添加收
  ExecDos::exec 'netsh advfirewall firewall add rule name="cameratest" dir=in program="$INSTDIR\${PRODUCT_NAME}.exe" action=allow'
  ExecDos::exec 'netsh advfirewall firewall add rule name="cameratest" dir=out program="$INSTDIR\${PRODUCT_NAME}.exe" action=allow'
  
SectionEnd

Section -Post
  WriteUninstaller "$INSTDIR\uninst.exe"
  WriteRegStr HKLM "${PRODUCT_DIR_REGKEY}" "" "$INSTDIR\cameratest.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayName" "$(^Name)"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "UninstallString" "$INSTDIR\uninst.exe"
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\cameratest.exe"
...
;
;以下是抄的老版本里面的写法
LangString SecCoreDesp ${LANG_SIMPCHINESE} "摄像头"
LangString SecCoreDesp ${LANG_ENGLISH} "cameratest"
...
Section Uninstall
  ...
  RMDir /r "$INSTDIR"
    ...
    ;防火墙规则删除
  ExecDos::exec 'netsh advfirewall firewall delete rule name="cameratest"'
  
  ;删除开始菜单和桌面的快捷方式
  SetShellVarContext all

  Delete "$SMPROGRAMS\cameratest\Uninstall.lnk"
  ;Delete "$SMPROGRAMS\cameratest\Website.lnk"
  Delete "$DESKTOP\$(ProgLinkName).lnk"
  Delete "$SMPROGRAMS\cameratest\$(ProgLinkName).lnk"

  RMDir /r "$SMPROGRAMS\cameratest"
  
  DeleteRegKey ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}"
  DeleteRegKey HKLM "${PRODUCT_DIR_REGKEY}"
  
  DeleteRegValue ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_AUTORUN_KEY}" "cameratest"
...
Function un.onInit
   ...
   FindProcDLL::FindProc "cameratest.exe"
   Pop $R0
   IntCmp $R0 1 0 no_run
   MessageBox MB_ICONSTOP $(RuningTip)
   Quit
